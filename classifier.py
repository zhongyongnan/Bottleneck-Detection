...
class RoiPoolingConv(Layer):
    ...
    def call(self, x, mask=None):
        ...
        rs          = tf.image.crop_and_resize(feature_map, tf.reshape(rois, [-1, 4]), box_index, (self.pool_size, self.pool_size), method='nearest')
...
class RoiAlignConv(Layer):
    def __init__(self, pool_size, **kwargs):
        self.pool_size = pool_size
        super(RoiAlignConv, self).__init__(**kwargs)
    ...
    def call(self, x, mask=None):
        ...
        rs = tf.image.crop_and_resize(feature_map, tf.reshape(rois, [-1, 4]), box_index, (self.pool_size, self.pool_size), method='bilinear')
...
def get_resnet50_classifier(base_layers, input_rois, roi_size, num_classes=21):
    ...
    out_roi_pool = RoiAlignConv(roi_size)([base_layers, input_rois])
